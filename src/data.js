import { indoorLocationData } from './indoorLocationData.js';

export const nodes = {
  "EX12": { lat: 10.879920826815594, lon: 77.01838731765748 },
  "EX11": { lat: 10.879994578697115, lon: 77.01876282691957 },
  "EX10": { lat: 10.880052515981843, lon: 77.01909542083742 },
  "EX9": { lat: 10.88011837948846, lon: 77.01936900615694 },
  "EX8": { lat: 10.880038030525, lon: 77.01941326260568 },
  "EX7": { lat: 10.879887892768833, lon: 77.0194534957409 },
  "EX6": { lat: 10.879711414958763, lon: 77.01949506998064 },
  "EX5": { lat: 10.87946250204905, lon: 77.01956883072855 },
  "EX4": { lat: 10.879274170979256, lon: 77.01960772275926 },
  "EX3": { lat: 10.879155640874329, lon: 77.01960772275926 },
  "EX2": { lat: 10.878836926358446, lon: 77.01962247490884 },
  "EX1": { lat: 10.878603215574085, lon: 77.01963670373154 },
  "BH4": { lat: 10.878447093264196, lon: 77.0190779864788 },
  "BH3": { lat: 10.878439191236737, lon: 77.01962381601335 },
  "TurfGround": { lat: 10.879575764033733, lon: 77.01934218406679 },
  "BH2": { lat: 10.8782930036911, lon: 77.0197592675686 },
  "BH1": { lat: 10.8781454990679, lon: 77.0198705792427 },
  "AH6": { lat: 10.878150767091418, lon: 77.02006235718729 },
  "AH5": { lat: 10.878020383482092, lon: 77.02007979154587 },
  "AH4": { lat: 10.877853123616946, lon: 77.02010259032251 },
  "AH3": { lat: 10.877670059562497, lon: 77.02011331915857 },
  "AH1": { lat: 10.877658204974725, lon: 77.02002078294755 },
  "AH2": { lat: 10.877658864994375, lon: 77.01990578323603 },
  "E14": { lat: 10.877332905326387, lon: 77.02012673020363 },
  "E15": { lat: 10.877350026449127, lon: 77.02034398913383 },
  "E12": { lat: 10.877310516164393, lon: 77.020732909441 },
  "E13": { lat: 10.87719988733929, lon: 77.02103465795518 },
  "E11": { lat: 10.877435632048078, lon: 77.02081203460695 },
  "E10": { lat: 10.877525188648388, lon: 77.02086433768272 },
  "E8": { lat: 10.8776331833365, lon: 77.02104605734348 },
  "E9": { lat: 10.877679278618288, lon: 77.0211647450924 },
  // eslint-disable-next-line no-loss-of-precision
  "E7": { lat: 10.877855757631295, lon: 77.02102661132812 },
  "E6": { lat: 10.878016432462733, lon: 77.02102795243265 },
  "AH10": { lat: 10.87818764325341, lon: 77.02103734016418 },
  "AH9": { lat: 10.878183692236265, lon: 77.02085629105568 },
  "AH8": { lat: 10.878177107207595, lon: 77.02070340514184 },
  "AH7": { lat: 10.87816130313816, lon: 77.02033191919328 },
  "C15": { lat: 10.878218592885837, lon: 77.02032051980495 },
  "C14": { lat: 10.878300247219826, lon: 77.02029135078193 },
  "SnacksBox": { lat: 10.878318685292147, lon: 77.02035941183567 },
  "C13": { lat: 10.878441825245918, lon: 77.02029705047607 },
  "FirstYearHostel": { lat: 10.878406266120107, lon: 77.01888889074327 },
  "C12": { lat: 10.878502407450494, lon: 77.02038623392583 },
  "C11": { lat: 10.878524796522976, lon: 77.02046334743501 },
  "C10": { lat: 10.878707860052595, lon: 77.02048346400261 },
  "C9": { lat: 10.87883429235275, lon: 77.02056527137758 },
  "C8": { lat: 10.87885668140032, lon: 77.02064573764802 },
  "C7": { lat: 10.878960066098141, lon: 77.02066384255886 },
  "C4": { lat: 10.878840218865497, lon: 77.02095553278924 },
  "E1": { lat: 10.878198179298844, lon: 77.02129483222963 },
  "E2": { lat: 10.878050345377154, lon: 77.02130187302829 },
  "E3": { lat: 10.877684546650038, lon: 77.02129483222963 },
  "E4": { lat: 10.877402706820613, lon: 77.02128410339357 },
  "E5": { lat: 10.877155108993648, lon: 77.02128142118455 },
  "R3": { lat: 10.878477384367482, lon: 77.02128008008005 },
  "R2": { lat: 10.878580110694848, lon: 77.0212706923485 },
  "R1": { lat: 10.878735517122832, lon: 77.02122777700426 },
  "C6": { lat: 10.878881704451489, lon: 77.02119156718256 },
  "FC": { lat: 10.8788790704462, lon: 77.02125594019891 },
  "F1": { lat: 10.879022623700436, lon: 77.02115535736085 },
  "C5": { lat: 10.879130617846135, lon: 77.02111378312112 },
  "C3": { lat: 10.87924387995692, lon: 77.02112451195717 },
  "C2": { lat: 10.879258696218232, lon: 77.02103566378356 },
  "C1": { lat: 10.879180334650062, lon: 77.0209612324834 },
  "CL1": { lat: 10.879252440463546, lon: 77.02034398913383 },
  "CL2": { lat: 10.879181980901718, lon: 77.02035538852216 },
  "CL3": { lat: 10.879197784917032, lon: 77.02062696218492 },
  "CL4": { lat: 10.879209967178257, lon: 77.02074632048608 },
  "CL5": { lat: 10.879229392945142, lon: 77.0209300518036 },
  "OpenAuditorium": { lat: 10.879324217009408, lon: 77.02072218060493 },
  "C-Lab": { lat: 10.879308413000798, lon: 77.02034130692483 },
  "M-Block": { lat: 10.880213849643328, lon: 77.0209689438343 },
  "E-Block": { lat: 10.877210423419633, lon: 77.0206792652607 },
  "Auditorium": { lat: 10.878569574662913, lon: 77.02112585306169 },
  "Store": { lat: 10.878472774851959, lon: 77.02121771872045 },
  "BoysHostel": { lat: 10.878192911276178, lon: 77.0214195549488 },
  "M6": { lat: 10.879516499047112, lon: 77.02109903097153 },
  "M4": { lat: 10.879685733477688, lon: 77.0210762321949 },
  "M3": { lat: 10.879947157681613, lon: 77.02103734016418 },
  // eslint-disable-next-line no-loss-of-precision
  "M1": { lat: 10.880215825138446, lon: 77.02102661132812 },
  "FirstYearGround": { lat: 10.878574842678924, lon: 77.01951250433922 },
  "TennisCourt": { lat: 10.87912403283837, lon: 77.0194521546364 },
  "M2": { lat: 10.880214279276725, lon: 77.02130287885667 },
  "L1": { lat: 10.880214255451648, lon: 77.0215053856373 },
  "LBlock": { lat: 10.880360224019858, lon: 77.0215067267418 },
  "EN5": { lat: 10.880246900243678, lon: 77.02189296483995 },
  "Library": { lat: 10.880447193400265, lon: 77.02177226543428 },
  "EN0": { lat: 10.880256571715382, lon: 77.02200964093208 },
  "EN1": { lat: 10.880304793218917, lon: 77.02233016490938 },
  "EN2": { lat: 10.880381633478317, lon: 77.02236235141756 },
  "EN3": { lat: 10.880446860511313, lon: 77.02242471277715 },
  "EN4": { lat: 10.880431047176591, lon: 77.0225226134062 },
  "EN6": { lat: 10.880287818615388, lon: 77.02238380908967 },
  "EN7": { lat: 10.880291982435864, lon: 77.02248439192773 },
  "EN8": { lat: 10.88033680074075, lon: 77.02253669500352 },
  "Entrance": { lat: 10.880440899736866, lon: 77.02258564531803 },
  "L2": { lat: 10.880124633237742, lon: 77.0215053856373 },
  "ML1": { lat: 10.880125948790944, lon: 77.02145442366601 },
  "L3": { lat: 10.880019358279087, lon: 77.02149063348772 },
  "ML2": { lat: 10.880016754245691, lon: 77.0214530825615 },
  "L4": { lat: 10.879904239910715, lon: 77.02149834483863 },
  "ML3": { lat: 10.879901604470499, lon: 77.02144302427769 },
  "L5": { lat: 10.879749197124072, lon: 77.02150136232378 },
  "L6": { lat: 10.87971455157038, lon: 77.02128410339357 },
  "ML4": { lat: 10.879797952066903, lon: 77.02121570706369 },
  "A3": { lat: 10.87975680382998, lon: 77.02179104089738 },
  "A2": { lat: 10.87979106456737, lon: 77.02207535505296 },
  "A1": { lat: 10.879948112399843, lon: 77.02205389738084 },
  "A4": { lat: 10.879889214313883, lon: 77.0225501060486 },
  "A5": { lat: 10.8800680009111, lon: 77.02252328395845 },
  "L7": { lat: 10.879703148368558, lon: 77.02154427766801 },
  "L8": { lat: 10.879303971778334, lon: 77.02158182859422 },
  "L9": { lat: 10.879269988222914, lon: 77.02156104147436 },
  "DBlock": { lat: 10.879192942997335, lon: 77.02149063348772 },
  "L10": { lat: 10.879248748709468, lon: 77.02125459909439 },
  "COE": { lat: 10.879195606540474, lon: 77.02125392854215 },
  "A7": { lat: 10.879248966023603, lon: 77.02167570590974 },
  "FC1": { lat: 10.878974668699255, lon: 77.02167302370073 },
  "A8": { lat: 10.879275317594333, lon: 77.02185675501823 },
  "ChemistryLab": { lat: 10.879214516066734, lon: 77.02186077833177 },
  "A9": { lat: 10.87929655772034, lon: 77.0220673084259 },
  "PhysicsLab": { lat: 10.879237117346078, lon: 77.02205792069437 },
  "A10": { lat: 10.879320264491714, lon: 77.02218264341356 },
  "A11": { lat: 10.879818094129867, lon: 77.02212363481523 },
  "A13": { lat: 10.87937637937702, lon: 77.02265605330467 },
  "A14": { lat: 10.879330799739312, lon: 77.0223355293274 },
  "SeminarHall": { lat: 10.879250785639075, lon: 77.02232748270035 },
  "A15": { lat: 10.879357138235596, lon: 77.0225501060486 },
  "NewHall": { lat: 10.879290268231674, lon: 77.02255547046663 },
  "B1": { lat: 10.879017088561785, lon: 77.02267080545425 },
  "BBlock": { lat: 10.879017128631368, lon: 77.02260173857213 },
  "B2": { lat: 10.878984820626428, lon: 77.02291488647462 },
  "B4": { lat: 10.878205284270726, lon: 77.0226949453354 },
  "B3": { lat: 10.878223732456599, lon: 77.02301144599916 },
  "B6": { lat: 10.878611144096023, lon: 77.02298998832704 },
  "GirlsHostel": { lat: 10.878047156916493, lon: 77.02298998832704 },
  "ABlock": { lat: 10.879522753382469, lon: 77.02188760042192 },
  "A17": { lat: 10.87949639894573, lon: 77.02157109975816 },
  "A18": { lat: 10.879543836930202, lon: 77.02215582132341 },
  "FoodCourt": { lat: 10.878829567387074, lon: 77.02150404453279 },
  "BH5": { lat: 10.878443800752768, lon: 77.01926708221436 },
  "BH6": { lat: 10.87844445925505, lon: 77.01908133924009 },
  "FirstYearGround2": { lat: 10.878581427698828, lon: 77.0192925632 },
  "Exit": { lat: 10.879848020666246, lon: 77.01803326606752 },
  "AcademicHall2": { lat: 10.877581992038936, lon: 77.01990276575089 },
  "AcademicHall1": { lat: 10.877587236580023, lon: 77.02001944184305 },
  "BoysHostel2": { lat: 10.87768773387326, lon: 77.02150404453279 },
  "Ragavendra": { lat: 10.878322200603058, lon: 77.02121973037721 },
  "Cblock": { lat: 10.879030615892397, lon: 77.02061891555788 },
  "c5": { lat: 10.879080647564965, lon: 77.02077448368074 },
  "NewPlacementCell": { lat: 10.8789647847315, lon: 77.02080130577089 },
  "Kabbadi Ground": { lat: 10.880342287111509, lon: 77.02127873897554 },
};
export const adjacency = {
  "EX12": ["EX11", "Exit"],
  "EX11": ["EX12", "EX10"],
  "EX10": ["EX11", "EX9"],
  "EX9": ["EX10", "EX8"],
  "EX8": ["EX9", "EX7"],
  "EX7": ["EX8", "EX6"],
  "EX6": ["EX7", "EX5", "TurfGround"],
  "EX5": ["EX6", "EX4"],
  "EX4": ["EX5", "EX3"],
  "EX3": ["EX4", "EX2", "TennisCourt"],
  "EX2": ["EX3", "EX1"],
  "EX1": ["EX2", "BH3"],
  "BH2": ["BH3", "BH1"],
  "BH1": ["BH2", "AH6"],
  "AH6": ["BH1", "AH5", "AH7"],
  "AH5": ["AH6", "AH4"],
  "AH4": ["AH5", "AH3"],
  "AH3": ["AH4", "AH1", "E14"],
  "AH1": ["AH3", "AH2", "AcademicHall1"],
  "AH2": ["AH1", "AcademicHall2"],
  "E14": ["AH3", "E15", "BH3"]
  ,
  "E15": ["E14", "E12"],
  "E12": ["E15", "E13", "E11", "E-Block"],
  "E13": ["E12", "E5"],
  "E11": ["E12", "E10"],
  "E10": ["E11", "E8"],
  "E8": ["E10", "E9", "E7"],
  "E9": ["E8", "E3"],
  "E7": ["E8", "E6"],
  "E6": ["E7", "AH10"],
  "AH10": ["E6", "AH9", "E1"],
  "AH9": ["AH10", "AH8"],
  "AH8": ["AH9", "AH7"],
  "AH7": ["AH8", "AH6", "C15"],
  "C15": ["AH7", "C14"],
  "C14": ["C15", "C13", "SnacksBox"],
  "SnacksBox": ["C14"],
  "C13": ["C14", "C12"],
  "FirstYearHostel": ["BH4"],
  "BH4": ["FirstYearHostel", "BH3"],
  "TurfGround": ["EX6"],
  "C12": ["C13", "C11"],
  "C11": ["C12", "C10"],
  "C10": ["C11", "C9"],
  "C9": ["C10", "C8"],
  "C8": ["C9", "C7", "C4"],
  "C7": ["C8", "Cblock"],
  "C4": ["C8", "C6"],
  "E1": ["AH10", "E2", "R3", "BoysHostel"],
  "E2": ["E3", "E1"],
  "E3": ["E4", "E9", "E2", "BoysHostel2"],
  "E4": ["E5", "E3"],
  "E5": ["E13", "E4"],
  "R3": ["R2", "E1", "Store"],
  "R2": ["R1", "R3", "Auditorium"],
  "R1": ["C6", "R2"],
  "C6": ["C4", "FC", "F1", "R1"],
  "FC": ["C6", "FoodCourt"],
  "F1": ["C5", "C6"],
  "C5": ["C3", "F1"],
  "C3": ["C5", "C2", "M6", "L9"],
  "C2": ["C3", "C1"],
  "C1": ["C2", "CL5"],
  "CL1": ["CL2", "C-Lab"],
  "CL2": ["CL3", "CL1"],
  "CL3": ["CL4", "CL2"],
  "CL4": ["CL5", "CL3", "OpenAuditorium", "c5"],
  "CL5": ["C1", "CL4"],
  "OpenAuditorium": ["CL4"],
  "C-Lab": ["CL1"],
  "Auditorium": ["R2"],
  "Store": ["R3", "Ragavendra"],
  "BoysHostel": ["E1"],
  "M6": ["C3", "M4"],
  "M4": ["M6", "M3", "L6"],
  "M3": ["M4", "M1"],
  "M1": ["M-Block", "M3", "M2"],
  "M-Block": ["M1"],
  "FirstYearGround": ["BH3"],
  "TennisCourt": ["EX3"],
  "E-Block": ["E12"],
  "M2": ["M1", "L1", "Kabbadi Ground"],
  "L1": ["M2", "LBlock", "EN5", "L2"],
  "LBlock": ["L1"],
  "EN5": ["L1", "Library", "EN0"],
  "Library": ["EN5"],
  "EN0": ["EN5", "EN1", "A1"],
  "EN1": ["EN0", "EN2", "EN6"],
  "EN2": ["EN1", "EN3"],
  "EN3": ["EN2", "EN4"],
  "EN4": ["EN3", "EN8", "Entrance"],
  "EN6": ["EN1", "EN7"],
  "EN7": ["EN6", "EN8", "A5"],
  "EN8": ["EN7", "EN4"],
  "Entrance": ["EN4"],
  "L2": ["L1", "ML1", "L3"],
  "ML1": ["L2"],
  "L3": ["L2", "ML2", "L4"],
  "ML2": ["L3"],
  "L4": ["ML3", "L3", "L5"],
  "ML3": ["L4"],
  "L5": ["L4", "L6", "A3", "L7"],
  "L6": ["L5", "ML4", "M4"],
  "ML4": ["L6"],
  "A3": ["L5", "A2", "ABlock"],
  "A2": ["A3", "A1", "A4", "A11"],
  "A1": ["A2", "EN0"],
  "A4": ["A2", "A5", "A11", "A13"],
  "A5": ["A4", "EN7"],
  "L7": ["L5", "L8"],
  "L8": ["L7", "L9", "A7"],
  "L9": ["C3", "L8", "DBlock"],
  "DBlock": ["L9"],
  "L10": ["COE"],
  "COE": ["L10"],
  "A7": ["L8", "FC1", "A8"],
  "FC1": ["A7", "FoodCourt"],
  "A8": ["A7", "ChemistryLab", "A9", "ABlock"],
  "ChemistryLab": ["A8"],
  "A9": ["A8", "PhysicsLab", "A10"],
  "PhysicsLab": ["A9"],
  "A10": ["A9", "A11", "A14"],
  "A11": ["A2", "A4", "A10", "ABlock"],
  "A13": ["A4", "A15", "B1"],
  "A14": ["A10", "SeminarHall", "A15"],
  "SeminarHall": ["A14"],
  "A15": ["A14", "A13", "NewHall"],
  "NewHall": ["A15"],
  "B1": ["A13", "BBlock", "B2", "B4"],
  "BBlock": ["B1"],
  "B2": ["B1", "B6"],
  "B4": ["B1", "B3"],
  "B3": ["B6", "B4", "GirlsHostel"],
  "B6": ["B2", "B3"],
  "GirlsHostel": ["B3"],
  "ABlock": ["A8", "A11", "A17", "A3", "A18"],
  "A17": ["ABlock"],
  "A18": ["ABlock"],
  "FoodCourt": ["FC1", "FC"],
  "BH5": ["BH4", "BH6", "FirstYearGround2"],
  "BH6": ["BH5", "FirstYearHostel"],
  "FirstYearGround2": ["BH5"],
  "Exit": ["EX12"],
  "AcademicHall2": ["AH2"],
  "AcademicHall1": ["AH1"],
  "Ragavendra": ["Store"],
  "Cblock": ["C7", "c5"],
  "c5": ["CL4", "Cblock", "NewPlacementCell"],
  "NewPlacementCell": ["c5"],
  "Kabbadi Ground": ["M2"],
  "BH3": ["BH4", "BH2", "FirstYearGround", "E14"],
  "BoysHostel2": ["E3"]
};

export const locationData = {
  "ABlock": {
    id: "ABlock",
    name: "Block A",
    category: "block",
    opens: 8,
    closes: 17,
    type: "destination",
    description: "Main academic block housing Computer Science and Electronics departments",
    facilities: ["Classrooms", "Computer Labs", "Faculty Rooms", "Seminar Halls"],
    departments: ["Computer Science", "Electronics & Communication"],
    indoorLocations: []
  },
  "BBlock": {
    id: "BBlock",
    name: "Block B",
    category: "block",
    opens: 8,
    closes: 17,
    type: "destination",
    description: "Academic block with modern classrooms and laboratories",
    facilities: ["Classrooms", "Labs", "Faculty Rooms"],
    departments: ["Mechanical", "Civil"],
    indoorLocations: []
  },
  "Cblock": {
    id: "Cblock",
    name: "Block C",
    category: "block",
    opens: 8,
    closes: 17,
    type: "destination",
    description: "Central academic block with multiple departments",
    facilities: ["Classrooms", "Computer Labs", "Study Areas"],
    departments: ["Information Technology", "Science & Humanities"],
    indoorLocations: []
  },
  "DBlock": {
    id: "DBlock",
    name: "Block D",
    category: "block",
    opens: 8,
    closes: 17,
    type: "destination",
    description: "Academic block for engineering departments",
    facilities: ["Classrooms", "Labs", "Project Rooms"],
    departments: ["Electrical", "Instrumentation"],
    indoorLocations: []
  },
  "E-Block": {
    id: "E-Block",
    name: "Block E",
    category: "block",
    opens: 8,
    closes: 17,
    type: "destination",
    description: "Engineering block with specialized laboratories",
    facilities: ["Classrooms", "Labs", "Workshops"],
    departments: ["Engineering"],
    indoorLocations: []
  },
  "M-Block": {
    id: "M-Block",
    name: "Block M",
    category: "block",
    opens: 8,
    closes: 17,
    type: "destination",
    description: "Multi-purpose academic block",
    facilities: ["Classrooms", "Labs", "Faculty Rooms"],
    departments: ["Multiple Departments"],
    indoorLocations: []
  },
  "Library": {
    id: "Library",
    name: "Central Library",
    category: "library",
    opens: 8,
    closes: 20,
    type: "destination",
    description: "Central library with extensive collection of books, journals, and digital resources",
    facilities: ["Reading Halls", "Digital Library", "Study Rooms", "Book Collection"],
    indoorLocations: []
  },
  "FoodCourt": {
    id: "FoodCourt",
    name: "Food Court",
    category: "food",
    opens: 7,
    closes: 21,
    type: "destination",
    description: "Main dining facility with variety of food options",
    facilities: ["Multiple Counters", "Seating Area", "Cafeteria"],
    indoorLocations: []
  },
  "Canteen": {
    id: "Canteen",
    name: "Canteen",
    category: "food",
    opens: 8,
    closes: 18,
    type: "destination",
    description: "Campus canteen serving meals and snacks",
    facilities: ["Dining Area", "Kitchen"],
    indoorLocations: []
  },
  "SnacksBox": {
    id: "SnacksBox",
    name: "Snacks Box",
    category: "food",
    opens: 9,
    closes: 18,
    type: "destination",
    description: "Quick snacks and beverages outlet",
    facilities: ["Snacks Counter", "Beverages"],
    indoorLocations: []
  },
  "Auditorium": {
    id: "Auditorium",
    name: "Main Auditorium",
    category: "hall",
    opens: 9,
    closes: 17,
    type: "destination",
    description: "Main auditorium for events, seminars, and cultural programs",
    facilities: ["Stage", "Seating", "Audio/Visual Equipment"],
    indoorLocations: []
  },
  "OpenAuditorium": {
    id: "OpenAuditorium",
    name: "Open Auditorium",
    category: "hall",
    opens: 0,
    closes: 24,
    type: "destination",
    description: "Open-air auditorium for outdoor events",
    facilities: ["Open Stage", "Seating Area"],
    indoorLocations: []
  },
  "AcademicHall1": { id: "AcademicHall1", name: "Academic Hall 1", category: "hall", opens: 9, closes: 17, type: "destination" },
  "AcademicHall2": { id: "AcademicHall2", name: "Academic Hall 2", category: "hall", opens: 9, closes: 17, type: "destination" },
  "NewHall": { id: "NewHall", name: "New Hall", category: "hall", opens: 9, closes: 17, type: "destination" },
  "SeminarHall": {
    id: "SeminarHall",
    name: "Seminar Hall",
    category: "hall",
    opens: 9,
    closes: 17,
    type: "destination",
    description: "Seminar hall for presentations and workshops",
    facilities: ["Projector", "Seating", "Audio System"],
    indoorLocations: []
  },
  "TurfGround": {
    id: "TurfGround",
    name: "Turf Ground",
    category: "sport",
    opens: 6,
    closes: 20,
    type: "destination",
    description: "Artificial turf ground for football and other sports",
    facilities: ["Turf Field", "Floodlights"],
    indoorLocations: []
  },
  "Kabbadi Ground": {
    id: "Kabbadi Ground",
    name: "Kabaddi Ground",
    category: "sport",
    opens: 6,
    closes: 20,
    type: "destination",
    description: "Dedicated ground for kabaddi and outdoor sports",
    facilities: ["Sports Field"],
    indoorLocations: []
  },
  "TennisCourt": {
    id: "TennisCourt",
    name: "Tennis Court",
    category: "sport",
    opens: 6,
    closes: 20,
    type: "destination",
    description: "Tennis court for students and staff",
    facilities: ["Tennis Court", "Equipment Storage"],
    indoorLocations: []
  },
  "GirlsHostel": {
    id: "GirlsHostel",
    name: "Girls Hostel",
    category: "hostel",
    opens: 0,
    closes: 24,
    type: "destination",
    description: "Residential facility for female students",
    facilities: ["Rooms", "Common Areas", "Mess"],
    indoorLocations: []
  },
  "BoysHostel": {
    id: "BoysHostel",
    name: "Boys Hostel",
    category: "hostel",
    opens: 0,
    closes: 24,
    type: "destination",
    description: "Residential facility for male students",
    facilities: ["Rooms", "Common Areas", "Mess"],
    indoorLocations: []
  },
  "Entrance": { id: "Entrance", name: "Main Entrance", category: "other", opens: 0, closes: 24, type: "destination" },
  "Exit": { id: "Exit", name: "Exit Gate", category: "other", opens: 0, closes: 24, type: "destination" },
  "ChemistryLab": {
    id: "ChemistryLab",
    name: "Chemistry Lab",
    category: "lab",
    opens: 9,
    closes: 17,
    type: "destination",
    description: "Chemistry laboratory with modern equipment",
    facilities: ["Lab Equipment", "Chemical Storage"],
    indoorLocations: []
  },
  "PhysicsLab": {
    id: "PhysicsLab",
    name: "Physics Lab",
    category: "lab",
    opens: 9,
    closes: 17,
    type: "destination",
    description: "Physics laboratory for experiments and research",
    facilities: ["Lab Equipment", "Instruments"],
    indoorLocations: []
  },
  "C-Lab": {
    id: "C-Lab",
    name: "Computer Lab",
    category: "lab",
    opens: 9,
    closes: 17,
    type: "destination",
    description: "Computer laboratory with latest systems and software",
    facilities: ["Computers", "Software", "Internet"],
    indoorLocations: []
  },
  "NewPlacementCell": {
    id: "NewPlacementCell",
    name: "Placement Cell",
    category: "office",
    opens: 9,
    closes: 17,
    type: "destination",
    description: "Placement and career guidance office",
    facilities: ["Counseling", "Interview Rooms"],
    indoorLocations: []
  },
  "COE": {
    id: "COE",
    name: "COE",
    category: "office",
    opens: 9,
    closes: 17,
    type: "destination",
    description: "Controller of Examinations office",
    facilities: ["Exam Records", "Administration"],
    indoorLocations: []
  },
  // Merge indoor locations (rooms, labs, halls, faculty rooms)
  ...indoorLocationData
};